...
FORCE_TIME=$(printf "%.1f" "$(echo "$TIMELIMIT + 0.3" | bc)")

# ======= 编译 =======
g++ "$SOURCE" -o "$EXEC" -O2 -std=c++14 -static 2> compile.log
if [ $? -ne 0 ]; then
    echo -e "${YELLOW}CE${NC}"
    cat compile.log
    exit 1
...
for INFILE in $(find "$DATADIR" -maxdepth 1 -name "*.in" | sort -V); do
    ...
    # ======= 更新最大值 =======
    cmp_t=$(awk -v t1="$TIME" -v t2="$maxt" 'BEGIN{print (t1 > t2) ? 1 : 0}')
    if [ "$cmp_t" -eq 1 ]; then maxt=$TIME; fi

    if [ "$MEM_MB" -gt "$maxm" ]; then maxm=$MEM_MB; fi
...
# 输出彩色 Score 行 + 最大值
echo -e "${NC}Score:${NC}\t\t     ${SCORE_COLOR}${passed}${NC} / ${GREEN}${total}${NC}\tmaxt = ${maxt}s\tmaxm = ${maxm}MB"